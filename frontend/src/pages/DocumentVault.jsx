...
// Utility to nicely format JSON keys (e.g., "card_number" -> "Card Number")
...
export default function DocumentVault() {
    ...
    const [selectedDocForExtraction, setSelectedDocForExtraction] = useState(null);
    ...
    // --- NEW: View Extraction Action ---
    async function handleViewExtraction(doc) {
        ...
        try {
            const data = await documentVaultService.getExtractedData(doc.id);
            setExtractionData(data);
        } catch (err) {
            setExtractionError(err.response?.data?.detail || "Extraction data is not available yet.");
        } finally {
    ...
    function getConfidenceClass(confidence) {
        if (!confidence && confidence !== 0) return 'confidence-medium';
    ...
    return (
        <div style={layoutStyles.layout}>
                            ...
                            <div className="document-grid">
                                {documents.map((document) => (
                                    <article className="finstack-card document-card" key={document.id}>
                                        ...
                                        {/* Action Buttons */}
                                        <div className="document-card-actions" style={{ flexWrap: 'wrap' }}>
                                            <button
                                            ...
                                            {/* Only show View Data if processing is completed */}
            ...
            {extractionModalOpen && (
                <div className="extraction-modal-overlay" onClick={() => setExtractionModalOpen(false)}>
                    <div className="extraction-modal-content" onClick={(e) => e.stopPropagation()}>
                        <div className="extraction-modal-header">
                            <h2 className="noto-serif">Extracted Financial Data</h2>
                            <button className="extraction-close-btn" onClick={() => setExtractionModalOpen(false)}>✕</button>
                        </div>
                        ...
                        <div className="extraction-modal-body">
                            {isExtractionLoading ? (
                                <div style={{ textAlign: 'center', padding: '40px 0' }}>
                                    <div className="vault-spinner"></div>
                                    <p className="text-secondary">Retrieving structured data...</p>
                                </div>
                            ) : extractionError ? (
                                <div className="vault-alert vault-alert-error" style={{ margin: 0 }}>
                                    <span className="alert-icon">⚠️</span>
                                    <span className="alert-text">{extractionError}</span>
                                </div>
                            ) : extractionData ? (
                                <div>
                                    ...
                                    <div className="extraction-meta">
                                        <div>
                                            ...
                                            <strong className="text-primary">{selectedDocForExtraction?.original_filename}</strong>
                                        </div>
                                        <div className={`confidence-badge ${getConfidenceClass(extractionData.confidence)}`}>
                                            🎯 Confidence: {extractionData.confidence ? `${extractionData.confidence}%` : 'N/A'}
                                        </div>
                                    </div>

                                    {/* Key/Value Data Grid */}
                                    <div className="extraction-data-grid">
                                        {/* Assuming backend returns fields inside extractionData.data or similar structure */}
                                        {Object.entries(extractionData.data || extractionData.fields || extractionData).map(([key, value]) => {
                                            // Skip rendering metadata keys if they are at the top level
                                            if (['confidence', 'errors', 'status', 'document_id'].includes(key)) return null;

                                            return (
                                                <div className="data-field" key={key}>
                                                    <span className="data-label">{formatKeyName(key)}</span>
                                    ...
                                    {extractionData.errors && extractionData.errors.length > 0 && (
                                        <div className="extraction-errors">
                                            <h4>Validation Issues Detected</h4>
                                            <ul>
                                                {extractionData.errors.map((err, idx) => (
                                                    <li key={idx}>{err}</li>
                                                ))}
                                            </ul>
                                        </div>
